...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_CREATE: 
		...
		break;
	case WM_KEYUP: // check key press messages
		switch(wparam)
		{
		case VK_ESCAPE:
			PostQuitMessage(0);
		...
		break;
	case WM_SIZE: // has the user resized the client area?
		{
...
int WINAPI WinMain(HINSTANCE hinstance, HINSTANCE hprevinstance, LPSTR lpcmdline, int ncmdshow)
{
	...
	while(!bDone)
	{
		while( PeekMessage( &msg, NULL, 0, 0, PM_REMOVE ) ) 
		{
			...
			{
				bDone = TRUE; // Stop loop if it's a quit message
			}
			else 
			{
		...
		{	
			if (!g_pController->Update())
			{
				...
				bDone = TRUE; // we have a problem, end app
			}
